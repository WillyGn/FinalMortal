...
import Triangle.AbstractSyntaxTrees.RecDeclaration;      // se importa RecDeclaration al Parser.
import Triangle.AbstractSyntaxTrees.PrivateDeclaration;  //se importa PrivateDeclaration al Parser
...
// to represent its phrase structure.
  ...
  Command parseElsifCommand() throws SyntaxError {
    Command commandAST = null; // in case there's a syntactic error
    SourcePosition commandPos = new SourcePosition();
    start(commandPos);//Al menos una vez
    accept(Token.ELSIF);
    ...
    if (currentToken.kind == Token.ELSIF) {
        commandAST = new ElsifCommand(eAST, c1AST, parseElsifCommand(), commandPos);
    }else{
        accept(Token.ELSE);
        Command elseCommand = parseCommand();
        commandAST = new ElsifCommand(eAST, c1AST, elseCommand, commandPos);
    }  
    return commandAST;
  }
    ...
    case Token.IF:
      {
        ...
        Command c1AST = parseCommand();
        if (currentToken.kind == Token.ELSIF){
            Command ifThenElse = parseElsifCommand();
            accept(Token.END);
            finish(commandPos);
            commandAST = new IfCommand(eAST, c1AST, ifThenElse, commandPos);
        }else{
            accept(Token.ELSE);
            Command elseCommand = parseCommand();
            accept(Token.END);
            finish(commandPos);
            commandAST = new IfCommand(eAST, c1AST, elseCommand, commandPos);
        }
...
//      break;
        ...
        switch(currentToken.kind){
            ...
            case Token.DO:
            {
                ...
                if(currentToken.kind == Token.WHILE){
                    accept(Token.WHILE);
                    Expression eASTW = parseExpression();
                    accept(Token.END);
                    finish(commandPos);
                    commandAST = new LoopDoWhileCommand(cAST,eASTW,commandPos);
                }else{
                    accept(Token.UNTIL);
                    Expression eASTU = parseExpression();
                    accept(Token.END);
                    finish(commandPos);
                    commandAST = new LoopDoUntilCommand(cAST,eASTU,commandPos);
                }
            }
            break;
            ...
            case Token.FOR:
            {
                ...
                accept(Token.END);
                finish(commandPos);
                commandAST = new LoopForCommand(iAST,eAST1,eAST2,cAST,commandPos);
            }
            break;
        }
      }
    ...
    //Eliminar de single-Command la primera alternativa (comando vacío)2
...
///////////////////////////////////////////////////////////////////////////////
  ...
  Declaration parseDeclaration() throws SyntaxError {
    ...
    declarationAST = parseCompoundDeclaration();                // SE UTILIZA COMPOUND DECLARATION EN LUGAR DE SINGLE DECLARATION
    while (currentToken.kind == Token.SEMICOLON) {
      acceptIt();
      Declaration d2AST = parseCompoundDeclaration();           // SE UTILIZA COMPOUND DECLARATION EN LUGAR DE SINGLE DECLARATION
...
//      break;  
    ...
    case Token.PROC:
      {
        ...
        accept(Token.END); // Agrego el end
        finish(declarationPos);
        declarationAST = new ProcDeclaration(iAST, fpsAST, cAST, declarationPos);
      }
      break;
  ...
  /*
  Deberiamos de hacer una declaracion Tipo ProcFunc Para el 
  ProcFunc de Abajo
  */
  ...
  Declaration parseCompoundDeclaration() throws SyntaxError {                   // SE CREA LA DECLARACIÓN COMPOUND DECLARATION
    ...
    switch (currentToken.kind) {
    case Token.REC:                                                             // SE CREA EL TOKEN REC, se agregó al abstract-syntax-tree el visitRecDeclaration
      {
        ...
        accept(Token.END);
        finish(declarationPos);
        declarationAST = new RecDeclaration(dAST, declarationPos);//A esta clase, en teoria
      }
      break;

    case Token.PRIVATE:                                                         // SE CREA EL TOKEN REC, se agregó al abstract-syntax-tree el visitPrivateDeclaration.
      {
        acceptIt();
        Declaration dAST = parseDeclaration();
        accept(Token.IN);
  ...
  Declaration parseProcFuncs() throws SyntaxError {                             // SE CREA PROCFUNCS DECLARATION, UTILIZANDO EL PRIVAMENTE CREADO PROC UNC
    ...
    finish(declarationPos);        
    if(currentToken.kind == Token.AND){                                         // SE CREA RECURSION PARA LA CREACION CONTINUA DE PROC FUNC
        accept(Token.AND);
    ...
    case Token.ARRAY:
      {
        acceptIt();
        IntegerLiteral ilAST = parseIntegerLiteral();
        accept(Token.DOUBLEDOT);                        // SE UTILIZA EL DOUBLE DOT
        IntegerLiteral ilAST2 = parseIntegerLiteral();  // SE AGREGA EL NUEVO PARSE INTEGER LITERAL
        accept(Token.OF);
        TypeDenoter tAST = parseTypeDenoter();
        finish(typePos);
